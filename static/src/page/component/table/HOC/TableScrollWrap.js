...
export class TableWrapCommon extends Component {
...
function TableScrollWrap(WrappedComponent) {
  return class extends TableWrapCommon {
    constructor(props) {
      ...
      this.tableComponentRef = createRef(null);
      this.scrollTableWraps = []
    }

    componentDidMount() {
      const { onAddLoopTableComponent } = this.props;
      onAddLoopTableComponent && onAddLoopTableComponent(this.tableComponentRef.current)
    ...
    getScrollTableWraps = () => {
      ...
      const { onAddLoopScrollTableWrap } = this.props;
      onAddLoopScrollTableWrap && onAddLoopScrollTableWrap(wraps)
    ...
    render() {
      return (
        <div ref={this.ref}>
          <WrappedComponent ref={this.tableComponentRef} {...this.props} onWrapScrollTo={this.handleWrapScrollTo} />
        </div>
      )
    }
  }
