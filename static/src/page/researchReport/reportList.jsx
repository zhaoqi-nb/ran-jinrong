...
class Index extends Component {
    ...
    goToDetail = (item) => {
        if (item.haveAccess) {
            ...
            message.info(i18n.format("无权限，请先申请查看"))
        }

    }

    //详情
    getReportInfoForGuest = async (item) => {
        const result = await Api.getReportInfoForGuest({ reportId: item.report_id });
        if (result.code == 200) {
                        ...
                        <List.Item
                            actions={item?.haveAccess ? [
                                ...
                                <div className="apply-btn" onClick={_.throttle(() => { this.updateReportApplyInfo(item) }, 2500, ({ leading: true, trailing: false }))}
                                    ...
                                    <span>{i18n.format(`${item?.status == "noPass" ? "重新申请" : "申请查看"}`)}</span>
                                </div>
                            ]}
                        >
                            <List.Item.Meta
